...
const LayoutPreview = ({
  ...
  const allPositions = useMemo(() => tables.map((it) => it.position), [tables]);
  const tableAtPosition = useCallback(
    (position) => tables.find((it) => it.position === position),
    [tables]
  );
  return (
      ...
      {[...Array(150)].map((x, i) => (
        <div key={i} className="layout-element">
          {allPositions.includes(i) ? (
            <div
              className="cell"
              onClick={() => layoutElementClickHandler(tableAtPosition(i))}
            >
              {draggable ? (
